/// Patched for blackmatter-ghostty: call metal/metallib directly from PATH
/// instead of via /usr/bin/xcrun. The Xcode metal stub delegates to a
/// cryptex-mounted binary that the Nix daemon can't access via xcrun.
/// Adding the cryptex bin/ to PATH allows direct invocation.
...
pub fn create(b: *std.Build, opts: Options) ?*MetallibStep {
    ...
    // Call metal directly from PATH instead of via xcrun
    ...
    // Call metallib directly from PATH instead of via xcrun
